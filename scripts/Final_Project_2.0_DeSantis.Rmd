...
# Now we will remove the STATION_STATUS and GIS_ID variables, as they have a lot of empty data that will become problematic later if not removed.
```{r}
...
# Now let's see what happens with the stations with a negative net gain. We'll pretty much do the exact same thing as we did with the positive net gain stations, first filtering, then joining with the metro location data, then mutating to distinguish the bikeshare and metro stations, and finally plotting on a map.
```{r}
