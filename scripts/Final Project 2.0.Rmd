...
```{r}
#df_count_start = df_count_start %>% rename(NAME = start_station_name)
bikeshare_squared = left_join(bikeshare, df_count_start, by = "NAME")
```

```{r}
bikeshare_squared2 = bikeshare_squared %>%
  select(-STATION_STATUS, -GIS_ID)
bikeshare_squared_b = na.omit(bikeshare_squared2)
```

```{r}
tmap_mode("view")
tm_shape(bikeshare_squared_b) + tm_dots("NAME", legend.show = FALSE) + tm_text("bikes_start",size = 0.5)
